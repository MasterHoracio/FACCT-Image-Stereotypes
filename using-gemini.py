...
def save_args(args, timestamp_now):
    path_file_args = os.path.join(args.model_path, "gemini-2.0-flash_outputs_args_" + args.type_approach + "_" + args.lang + "_" + timestamp_now)
    dump_json(vars(args), path_file_args)

def get_response(model, prompt, tokens, images=None):
    if images is None:
...
def run_generation(lang, questions, timestamp_now, model, model_path, type_approach, is_images=False, stereotype_list = None):
    ...
    for question in tqdm(questions, desc="Processing questions"):
        ...
        response = get_response(model, prompt_question, 256, images)
    ...
    if is_images:
        file_name =  "gemini-2.0-flash_outputs_" + type_approach + "_" + lang + "_" + fixed_part_type_generation + "_" + timestamp_now
    else:
        file_name =  "gemini-2.0-flash_outputs_" + lang + "_" + fixed_part_type_generation + "_" + timestamp_now
...
def main(args):
    ...
    run_generation(lang, textual_questions, timestamp_now, model, args.model_path, type_approach, False)

    #we now ask to rate the images one by one
    images_questions = load_image_questions(DIR_IMAGES, question_img, type_approach)
    if type_approach == "cot":
        stereotype_list = get_response(model, images_questions[0]["prompt_list"], 512, None)
        run_generation(lang, images_questions, timestamp_now, model, args.model_path, type_approach, True, stereotype_list)
    elif type_approach == "zero" or type_approach == "guided":
        run_generation(lang, images_questions, timestamp_now, model, args.model_path, type_approach, True)

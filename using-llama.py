...
def load_image_questions(dir_path, question_img, type_approach = None):
    ...
    if type_approach == "zero" or type_approach == "guided" or type_approach is None:
        image_questions = [{"name": f_name.split(".")[0], 
                    ...
                    "prompt": format_img_question.format(question_img),
...
def save_args(args, timestamp_now):
    path_file_args = os.path.join(args.model_path, "Llama-3.2-11B-Vision_args_" + args.type_approach + "_" + args.lang + "_" + timestamp_now + ".json")
    dump_json(vars(args), path_file_args)

def get_response(processor, model, prompt, tokens, images=None):
    ...
    with torch.inference_mode():
        outputs = model.generate(**inputs_question, 
...
def run_generation(lang, questions, processor, model, timestamp_now, model_path, type_approach, is_images = False, stereotype_list = None):
    ...
    for question in tqdm(questions, desc="Processing questions"):
        ...
        decoded = extract_answer(get_response(processor, model, prompt_question, 256, images)[0], "ASSISTANT:")
        question["answer"] = [decoded]
        
    fixed_part_type_generation = "images" if is_images else "textual"
    
    if is_images:
        file_name =  "Llama-3.2-11B-Vision_outputs_" + type_approach + "_" + lang + "_" + fixed_part_type_generation + "_" + timestamp_now + ".json"
    else:
        file_name =  "Llama-3.2-11B-Vision_outputs_" + lang + "_" + fixed_part_type_generation + "_" + timestamp_now + ".json"
...
def main(args):
    ...
    if type_approach == "cot":
        ...
        stereotype_list = extract_answer(get_response(processor, model, prompt, 512, None)[0], "ASSISTANT:")
        run_generation(lang, images_questions, processor, model, timestamp_now, args.model_path, type_approach, True, stereotype_list)
